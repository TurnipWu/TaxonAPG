...
plot_phylo <- function(PhyloTree,NoteColor=c("#f5c88f","#b3c384","#73a66b")){
  ...
  PhyloData <- ggplot2::fortify(PhyloTree[[1]])|>
    ...
    dplyr::rename("NoteType"=output.note)

  TreeRes <- ggtree::ggtree(PhyloData,
                            layout = "circular",
                            branch.length = "none",color=NA,
                            linetype=1,size=2,ladderize = T,
                            show.legend=F)+
    ggtree::geom_tree(ggplot2::aes(color=NoteType),
                      linetype=1,size=2,alpha=0.75)+
    ggtree::geom_tiplab(ggplot2::aes(color=NoteType,label=label),
                        color="#484848",
                        fontface="italic",
                        offset = 0.05,
                        hjust=-0.14,
                        show.legend=F,
                        size=5)+
    ggtree::geom_tippoint(ggplot2::aes(color=NoteType,shape=NoteType),
                          size=4)+
    ggplot2::xlim(-50,NA)+
    ggplot2::scale_color_manual(values=NoteColor)+
